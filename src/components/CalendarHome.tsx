...
export default function CalendarHome() {
  ...
  async function reload(keepMonth = false) {
    ...
    if (!keepMonth && list.length) setMonth(list[list.length - 1].dateKst.slice(0, 7));
  }
  useEffect(() => { reload(); }, []);
  ...
  const monthDays = useMemo(() => days.filter((d) => d.dateKst.startsWith(month)).slice().reverse(), [days, month]);
  ...
  async function syncDates(dates: string[]) {
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      const res = await fetch("/api/sync", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ date }) });
      const json = await res.json();
      if (!res.ok) { setSyncMsg(json.error || "failed"); setSyncBusy(false); return; }
      ok += 1;
      added += json.synced || 0;
    }
    setSyncBusy(false);
  ...
  async function syncToday() {
    ...
    setMonth(kstDateKey().slice(0, 7));
    setOpen(kstDateKey());
  }
  async function syncMonth() {
    const todayKey = kstDateKey();
    const last = month === todayKey.slice(0, 7) ? Number(todayKey.slice(8, 10)) : count;
    await syncDates(Array.from({ length: last }, (_, i) => `${month}-${String(i + 1).padStart(2, "0")}`));
  }
  ...
  async function saveNote() {
    if (!noteDate) return;
    await fetch("/api/day-note", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ date: noteDate, note: noteText }) });
    setNoteDate(null);
  }

  const cards = monthDays.length ? monthDays : today.startsWith(month) ? [{ dateKst: today, trades: 0, wins: 0, losses: 0, winRate: 0, realizedPnl: 0, fee: 0, fundingFee: 0, netPnl: 0, avgLeverage: null } as DayStats] : [];
    ...
    <main className="mx-auto max-w-6xl overflow-x-hidden px-4 py-8">
      <div className="mb-3 grid items-end gap-6 lg:grid-cols-[minmax(0,1fr)_240px]">
        <div className="flex items-end justify-between gap-3">
          ...
          <button onClick={syncToday} disabled={syncBusy} className="rounded-lg bg-[#6d5cff] px-4 py-2 text-sm text-white disabled:opacity-50">{syncBusy ? "IMGBB…" : "IMGBB"}</button>
        </div>
        <div />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_240px]">
        <div className="min-w-0 space-y-3">
          {cards.map((st) => {
            ...
            let c = 0;
            const pts = [{ label: "start", value: 0 }, ...chrono.map((t, i) => { c += t.netPnl; return { label: `#${i + 1}`, value: c }; })];
            return (
              <section key={st.dateKst} className="overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3">
                  <button onClick={() => setOpen(expanded ? "" : st.dateKst)} className="flex flex-wrap items-center gap-2 text-left">
                    ...
                    <span className={`font-semibold ${st.netPnl >= 0 ? "text-[#16a34a]" : "text-[#ef4444]"}`}>Net P&L {money(st.netPnl)}</span>
                  ...
                  <button onClick={() => openNote(st.dateKst)} className="rounded-full border border-[#e5e7eb] px-3 py-1 text-xs text-[#6b7280]">+ Add note</button>
                </div>
                <div className="grid gap-4 border-t border-[#f3f4f6] px-4 py-3 md:grid-cols-[280px_1fr]">
                  ...
                  <div className="max-w-full overflow-x-auto border-t border-[#f3f4f6]">
                    <table className="w-full min-w-[900px] text-left text-sm">
                      <thead className="text-[#6b7280]">
                        <tr>{["Ticker", "Side", "Instrument", "Net P&L", "Net ROI", "Closed P&L", "Fee", "Funding", "Lev", "Size", "Time"].map((h) => <th key={h} className="px-3 py-2 font-medium">{h}</th>)}</tr>
                      </thead>
                      <tbody>
                        {list.length === 0 ? (
                          <tr><td colSpan={11} className="px-3 py-6 text-center text-[#6b7280]">No trades</td></tr>
                        ) : list.map((t) => {
                          const tk = tickerOf(t.instId);
                          return (
                            <tr key={t.id} className="border-t border-[#f3f4f6]">
                              <td className="px-3 py-2"><Link href={`/journal/${t.dateKst}`} className="rounded-full bg-[#f3f4f6] px-2 py-0.5 text-xs">{tk}</Link></td>
                              ...
                              <td className="px-3 py-2 text-[#111827]">{roiPct(roi(t))}</td>
                              <td className="px-3 py-2 text-[#111827]">{money(closedPnl(t))}</td>
                              ...
                              <td className="px-3 py-2 whitespace-nowrap text-[#6b7280]">{new Date(t.closedAt).toLocaleTimeString("en-GB", { timeZone: "Asia/Seoul", hour12: false })}</td>
                            </tr>
                          );
                        })}
                      </tbody>
        ...
        <aside className="h-fit w-full rounded-2xl border border-[#e5e7eb] bg-white p-3 shadow-sm lg:sticky lg:top-4">
          <div className="mb-2 flex items-center justify-between gap-1">
            <button type="button" onClick={() => setMonth(shiftMonth(month, -1))} className="h-7 w-7 rounded text-[#6b7280] hover:bg-[#f3f4f6]">‹</button>
            <div className="text-sm font-medium">{month}</div>
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => setMonth(shiftMonth(month, 1))} className="h-7 w-7 rounded text-[#6b7280] hover:bg-[#f3f4f6]">›</button>
              <button type="button" onClick={syncMonth} disabled={syncBusy} className="flex h-7 w-7 items-center justify-center rounded-full bg-[#6d5cff] text-white disabled:opacity-50">+</button>
            </div>
          </div>
          <div className="grid grid-cols-7 gap-1 text-center text-[11px] text-[#9ca3af]">
            ...
            {Array.from({ length: count }).map((_, i) => {
              ...
              return (
                <button key={key} type="button" onClick={() => setOpen(key)} className={`rounded py-1 ${key === today ? "ring-1 ring-[#6d5cff] " : ""}${win ? "bg-[#dcfce7] text-[#16a34a]" : loss ? "bg-[#fee2e2] text-[#ef4444]" : "text-[#9ca3af]"}`}>
                  {i + 1}
                </button>
              );
            })}
      ...
      {noteDate ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setNoteDate(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-3"><RichText value={noteText} onChange={setNoteText} /></div>
