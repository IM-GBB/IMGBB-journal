...
export default function CalendarHome() {
  ...
  const cards = monthDays.length
    ...
    : today.startsWith(month)
      ? [{ dateKst: today, trades: 0, wins: 0, losses: 0, winRate: 0, realizedPnl: 0, fee: 0, fundingFee: 0, netPnl: 0, avgLeverage: null } as DayStats]
      : [];
        ...
        <aside className="h-fit w-full rounded-2xl border border-[#e5e7eb] bg-white p-3 shadow-sm lg:sticky lg:top-4">
          <div className="mb-2 flex items-center justify-between gap-1">
            ...
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => setMonth(shiftMonth(month, 1))} className="h-7 w-7 rounded text-[#6b7280] hover:bg-[#f3f4f6]">›</button>
              <button type="button" onClick={syncMonth} disabled={syncBusy} title="이 달 동기화" className="flex h-7 w-7 items-center justify-center rounded-full bg-[#6d5cff] text-white disabled:opacity-50">+</button>
            </div>
          </div>
            ...
            {Array.from({ length: count }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setOpen(key)}
                  className={`rounded py-1 ${key === today ? "ring-1 ring-[#6d5cff] " : ""}${win ? "bg-[#dcfce7] text-[#16a34a]" : loss ? "bg-[#fee2e2] text-[#ef4444]" : "text-[#9ca3af]"}`}
                >
                  {i + 1}
                </button>
              );
      ...
      {noteDate ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setNoteDate(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <textarea className="mt-3 h-36 w-full rounded-lg border border-[#e5e7eb] px-3 py-2 text-sm" value={noteText} onChange={(e) => setNoteText(e.target.value)} />
