...
function MiniCurve({ trades }: { trades: Trade[] }) {
  ...
  if (!pts.length) {
    return <div className="h-16 w-28 rounded bg-[#1b2028]" />;
  }
    ...
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_240px]">
        <div className="space-y-3">
          {cards.length === 0 ? (
            <p className="rounded-xl border border-[#2a313c] bg-[#14181e] px-4 py-10 text-center text-sm text-[#8b95a5]">
              이 달 기록 없음. 동기화하세요.
            </p>
          ) : null}
            ...
            return (
              <section key={st.dateKst} className="overflow-hidden rounded-xl border border-[#2a313c] bg-[#14181e]">
                ...
                >
                  ...
                  <span className={`font-medium ${pos ? "text-[#3dd68c]" : "text-[#f07178]"}`}>
                    Net {won(st.netPnl)}
                  </span>
                </button>
